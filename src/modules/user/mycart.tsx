import React, { useState, useCallback } from "react"; // Bỏ useEffect vì không cần sync selection nữa
import { Link, useNavigate } from "react-router-dom";
import { useTranslation } from 'react-i18next';
import {
  ArrowLeftIcon,
  HeartIcon,
  TrashIcon,
} from "@heroicons/react/24/outline"; // Bỏ ShieldCheckIcon nếu không dùng chỗ nào khác
import UserLayout from "./layout/layout";
import { useCart } from "./hooks/useCart";
import { cartService } from "./services/cart.service";

// Dữ liệu tĩnh giữ nguyên
type RecommendedCourse = {
  id: string;
  title: string;
  instructor: string;
  image: string;
  price: number;
  originalPrice?: number;
};
const recommendedCourses: RecommendedCourse[] = [
    {
      id: "css-sass",
      title: "Chuyên sâu CSS và Sass",
      instructor: "Emma Davis",
      image: "https://images.unsplash.com/photo-1521572267360-ee0c2909d518?auto=format&fit=crop&w=900&q=80",
      price: 549_000,
      originalPrice: 899_000,
    },
    {
      id: "mongodb",
      title: "MongoDB Database Course",
      instructor: "Alex Wilson",
      image: "https://images.unsplash.com/photo-1454165205744-3b78555e5572?auto=format&fit=crop&w=900&q=80",
      price: 629_000,
      originalPrice: 1_099_000,
    },
];
const currencyFormatter = new Intl.NumberFormat("vi-VN", {
  style: "currency",
  currency: "VND",
  maximumFractionDigits: 0,
});
const formatCurrency = (value: number) => currencyFormatter.format(value);

const MyCart: React.FC = () => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const { cart, loading, error, refreshCart } = useCart();

  const paymentMethods = [
    {
      value: "card",
      label: t('cart.securePayment.creditCard'),
      description: t('cart.securePayment.creditCardDesc'),
    },
    {
      value: "paypal",
      label: t('cart.securePayment.paypal'),
      description: t('cart.securePayment.paypalDesc'),
    },
    {
      value: "gift",
      label: t('cart.securePayment.giftCard'),
      description: t('cart.securePayment.giftCardDesc'),
    },
  ];

  const [selectedPayment, setSelectedPayment] = useState<string>(paymentMethods[0]?.value ?? "card");
  const [promoCode, setPromoCode] = useState("");
  // ĐÃ BỎ: State selectedIds
  const [deletingId, setDeletingId] = useState<string | null>(null);

  // ĐÃ BỎ: useEffect sync selectedIds

  // THAY ĐỔI: Lấy tất cả items trong giỏ hàng thay vì items được chọn
  const validCartItems = React.useMemo(
    () => cart?.items ?? [],
    [cart]
  );

  const handleRemove = useCallback(async (id: string) => {
    setDeletingId(id);
    try {
        const response = await cartService.removeCourseFromCart(id);
        if (response.success) {
            refreshCart();
        } else {
            alert(`${t('common.error')}: ${response.message || t('course.removeError')}`);
        }
    } catch (err) {
        console.error("Lỗi khi xóa khóa học:", err);
        alert(t('course.removeConnectionError'));
    } finally {
        setDeletingId(null);
    }
  }, [refreshCart]);

  // ĐÃ BỎ: handleToggleSelect

  const handleAddRecommended = useCallback(async (_course: RecommendedCourse) => {
      // await cartService.addCourseToCart(course.id);
      refreshCart();
  }, [refreshCart]);

  // THAY ĐỔI: Tính tổng dựa trên toàn bộ items
  const subtotal = validCartItems.reduce((sum, item) => sum + item.price, 0);
  const total = subtotal;

  const handleCheckout = () => {
    navigate("/user/checkout");
  };

  if (loading) {
    return (
        <UserLayout>
            <div className="flex min-h-[60vh] items-center justify-center text-center">{t('cart.loading')}</div>
        </UserLayout>
    );
  }

  if (error) {
      return (
          <UserLayout>
              <div className="flex min-h-[60vh] items-center justify-center text-center text-red-500">{error}</div>
          </UserLayout>
      );
  }

  return (
    <UserLayout>
      <div className="space-y-8">
        <Link
          to="/courses"
          className="inline-flex items-center gap-2 text-sm font-semibold text-[#5a2dff] transition hover:text-[#3c1cd6]"
        >
          <ArrowLeftIcon className="h-4 w-4" />
          {t('cart.continueShopping')}
        </Link>

        <header className="space-y-2">
          <h1 className="text-3xl font-bold text-gray-900">{t('cart.title')}</h1>
          <p className="text-sm text-gray-500">{t('cart.itemCount', { count: cart?.totalItems ?? 0 })}</p>
        </header>

        {cart && cart.items.length > 0 ? (
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr),360px]">
              <section className="space-y-6">
                {cart.items.map((item) => {
                  const detailPath = `/courses/${item.id}`;
                  const isDeleting = deletingId === item.id;
                  
                  return (
                    <article
                      key={item.id}
                      // THAY ĐỔI: Bỏ logic border theo selection, để mặc định border-gray-100
                      className={`rounded-3xl border border-gray-100 bg-white p-6 shadow-[0_20px_50px_rgba(15,23,42,0.08)]`}
                    >
                      <div className="flex flex-col gap-6 md:flex-row">
                        <div className="flex items-start gap-4">
                          {/* ĐÃ BỎ: thẻ <input type="checkbox" ... /> ở đây */}
                          
                          <Link
                            to={detailPath}
                            className="relative h-40 w-full overflow-hidden rounded-2xl md:w-48"
                          >
                            <img src={item.imageUrl} alt={item.name} className="h-full w-full object-cover" />
                          </Link>
                        </div>

                        <div className="flex flex-1 flex-col justify-between gap-4">
                          <div className="space-y-3">
                            <div className="space-y-1">
                              <h2 className="text-lg font-semibold text-gray-900">
                                <Link to={detailPath} className="transition hover:text-[#5a2dff]">
                                  {item.name}
                                </Link>
                              </h2>
                              <p className="text-sm text-gray-500">{t('cart.item.by')} {item.instructorName}</p>
                            </div>
                          </div>

                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-baseline gap-2">
                              <span className="text-xl font-semibold text-[#5a2dff]">
                                {formatCurrency(item.price)}
                              </span>
                            </div>
                            <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-gray-600">
                              <button
                                type="button"
                                className="inline-flex items-center gap-1 rounded-full border border-gray-200 px-4 py-2 transition hover:border-[#5a2dff] hover:text-[#5a2dff]"
                              >
                                <HeartIcon className="h-4 w-4" />
                                {t('cart.item.saveForLater')}
                              </button>

                              <button
                                type="button"
                                onClick={() => handleRemove(item.id)}
                                disabled={isDeleting}
                                className="inline-flex items-center gap-1 rounded-full border border-gray-200 px-4 py-2 transition hover:border-red-400 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                <TrashIcon className="h-4 w-4" />
                                {isDeleting ? t('cart.item.removing') : t('cart.item.remove')}
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    </article>
                  );
                })}

                <section className="rounded-3xl border border-gray-100 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.08)]">
                  <h3 className="text-lg font-semibold text-gray-900">
                    {t('cart.recommendations.title')}
                  </h3>
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    {recommendedCourses.map((course) => (
                      <article
                        key={course.id}
                        className="flex flex-col justify-between gap-4 rounded-2xl border border-gray-100 p-4 transition hover:border-[#5a2dff]"
                      >
                        <div className="flex items-center gap-4">
                          <img
                            src={course.image}
                            alt={course.title}
                            className="h-20 w-28 rounded-xl object-cover"
                          />
                          <div className="space-y-1 text-sm">
                            <h4 className="font-semibold text-gray-900 line-clamp-2">{course.title}</h4>
                            <p className="text-xs text-gray-500">{t('cart.item.by')} {course.instructor}</p>
                            <div className="flex items-baseline gap-2">
                              <span className="text-base font-semibold text-[#5a2dff]">
                                {formatCurrency(course.price)}
                              </span>
                              {course.originalPrice && (
                                <span className="text-xs text-gray-400 line-through">
                                  {formatCurrency(course.originalPrice)}
                                </span>
                              )}
                            </div>
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => handleAddRecommended(course)}
                          className="w-full rounded-full bg-[#05001a] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#1e0b63]"
                        >
                          {t('cart.recommendations.addToCart')}
                        </button>
                      </article>
                    ))}
                  </div>
                </section>
              </section>

              <aside className="space-y-6">
                <div className="rounded-3xl bg-white p-6 shadow-[0_24px_56px_rgba(15,23,42,0.08)]">
                  <h2 className="text-lg font-semibold text-gray-900">{t('cart.orderSummary.title')}</h2>
                  <div className="mt-4 space-y-3 text-sm">
                    <div className="flex items-center justify-between text-gray-500">
                      {/* THAY ĐỔI: Hiển thị tổng số lượng item thay vì item đã chọn */}
                      <span>{t('cart.orderSummary.subtotal', { count: validCartItems.length })}</span>
                      <span className="font-semibold text-gray-900">{formatCurrency(subtotal)}</span>
                    </div>
                  </div>
                  <div className="my-4 h-px bg-gray-100" />
                  <div className="flex items-center justify-between text-lg font-semibold text-gray-900">
                    <span>{t('cart.orderSummary.total')}</span>
                    <span>{formatCurrency(total)}</span>
                  </div>
                  <button
                    type="button"
                    onClick={handleCheckout}
                    // THAY ĐỔI: Disable nếu giỏ hàng rỗng (không phải selection rỗng)
                    disabled={validCartItems.length === 0}
                    className={`mt-6 w-full rounded-full px-4 py-3 text-sm font-semibold text-white shadow-[0_14px_34px_rgba(90,45,255,0.35)] transition ${
                      validCartItems.length === 0
                        ? "cursor-not-allowed bg-[#5a2dff]/60"
                        : "bg-[#5a2dff] hover:bg-[#4a21eb]"
                    }`}
                  >
                    {t('cart.orderSummary.checkout')}
                  </button>
                </div>
                
                <div className="rounded-3xl bg-white p-6 shadow-[0_24px_56px_rgba(15,23,42,0.08)]">
                  <h3 className="text-lg font-semibold text-gray-900">{t('cart.promoCode.title')}</h3>
                  <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                    <input
                      type="text"
                      value={promoCode}
                      onChange={(event) => setPromoCode(event.target.value)}
                      placeholder={t('cart.promoCode.placeholder')}
                      className="h-11 flex-1 rounded-full border border-gray-200 bg-gray-50 px-4 text-sm font-medium text-gray-600 outline-none transition focus:border-[#5a2dff] focus:bg-white"
                    />
                    <button
                      type="button"
                      className="h-11 rounded-full border border-[#5a2dff] px-5 text-sm font-semibold text-[#5a2dff] transition hover:bg-[#efe7ff]"
                    >
                      {t('cart.promoCode.apply')}
                    </button>
                  </div>
                </div>

                <div className="rounded-3xl bg-white p-6 shadow-[0_24px_56px_rgba(15,23,42,0.08)]">
                  <h3 className="text-lg font-semibold text-gray-900">{t('cart.securePayment.title')}</h3>
                  <div className="mt-4 space-y-3">
                    {paymentMethods.map((method) => (
                      <label
                        key={method.value}
                        className={`flex cursor-pointer items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-semibold transition ${
                          selectedPayment === method.value
                            ? "border-[#5a2dff] bg-[#f4f0ff] text-[#5a2dff]"
                            : "border-gray-200 text-gray-600 hover:border-[#5a2dff]/60 hover:text-[#5a2dff]"
                        }`}
                      >
                        <input
                          type="radio"
                          name="payment"
                          value={method.value}
                          checked={selectedPayment === method.value}
                          onChange={() => setSelectedPayment(method.value)}
                          className="h-4 w-4 text-[#5a2dff]"
                        />
                        <div>
                          <span>{method.label}</span>
                          {method.description && (
                            <p className="text-xs font-normal text-gray-400">{method.description}</p>
                          )}
                        </div>
                      </label>
                    ))}
                  </div>
                </div>

              </aside>
            </div>
        ) : (
            <div className="text-center p-12 border-2 border-dashed rounded-3xl">
                <h2 className="text-xl font-semibold">{t('cart.empty.title')}</h2>
                <p className="mt-2 text-gray-500">{t('cart.empty.subtitle')}</p>
                <Link to="/courses" className="mt-6 inline-block rounded-full bg-[#5a2dff] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#4a21eb]">
                    {t('cart.empty.exploreCourses')}
                </Link>
            </div>
        )}
      </div>
    </UserLayout>
  );
};

export default MyCart;